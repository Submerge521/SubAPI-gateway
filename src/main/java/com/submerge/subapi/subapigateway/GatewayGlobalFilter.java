package com.submerge.subapi.subapigateway;

import com.google.gson.Gson;
import com.google.gson.reflect.TypeToken;
import com.submerge.subapi.subapigateway.exception.BusinessException;
import com.submerge.subapi.subapigateway.utils.RedissonLockUtil;
import com.submerge.subapiclientsdk.utils.GenSignUtils;
import com.submerge.subapicommon.common.ErrorCode;
import com.submerge.subapicommon.model.dto.RequestParamsField;
import com.submerge.subapicommon.model.emums.InterfaceStatusEnum;
...
import static com.submerge.subapi.subapigateway.CacheBodyGatewayFilter.CACHE_REQUEST_BODY_OBJECT_KEY;
import static com.submerge.subapi.subapigateway.utils.NetUtils.getIp;
import static com.submerge.subapicommon.model.emums.UserAccountStatusEnum.BAN;
...
//    private final static List<String> WHITE_HOST_LIST = Arrays.asList("127.0.0.1", "101.43.61.87");
    ...
    @DubboReference
    private InnerUserInterfaceInfoService interfaceInvokeService;
    @DubboReference
...
//        String body = URLDecoder.decode(Objects.requireNonNull(headers.getFirst("body")), "utf-8");
        ...
        try {
            ...
            // 校验签名
            if (!GenSignUtils.genSign(body, user.getSecretKey()).equals(sign)) {
                    ...
                    for (RequestParamsField requestParamsField : list) {
                        if ("是".equals(requestParamsField.getRequired())) {
                            if (StringUtils.isBlank((CharSequence) requestBodyMap.get(requestParamsField.getFieldName())) || !requestBodyMap.containsKey(requestParamsField.getFieldName())) {
                                throw new BusinessException(ErrorCode.FORBIDDEN_ERROR, "请求参数有误，" + requestParamsField.getFieldName() + "为必选项，详细参数请参考API文档：https://doc.Submergeu.icu/");
                            }
                        }
                    }
                }
                        ...
                        return super.writeWith(
                                fluxBody.map(dataBuffer -> {
                                    ...
                                    redissonLockUtil.redissonDistributedLocks(("gateway_" + user.getUserAccount()).intern(), () -> {
                                        boolean invoke = interfaceInvokeService.invokeCount(interfaceInfo.getId(), user.getId(), interfaceInfo.getReduceScore());
                                        if (!invoke) {
